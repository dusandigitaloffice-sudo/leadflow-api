...
// Health
app.get('/', (req, res) => res.json({ status: 'ok', service: 'leadflow-api', version: '2.0.0' }));
...
app.post('/api/auth/signup', async (req, res) => {
  try {
    ...
    res.json({ user: { id: data.user.id, email: data.user.email }, token: signIn.session.access_token, refresh_token: signIn.session.refresh_token });
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    if (error) throw error;
    res.json({ user: { id: data.user.id, email: data.user.email }, token: data.session.access_token, refresh_token: data.session.refresh_token });
  } catch (err) { res.status(400).json({ error: err.message }); }
});
...
app.post('/api/public/submit/:formId', async (req, res) => {
  try {
...
app.listen(PORT, () => console.log(`LeadFlow API v2 on port ${PORT}`));
